...
const RolesPermissionsForm = () => {
  ...
  const moduleData = useSelector((state: TRootState) => state.rolesAndPermission?.modulesData?.modules);

  const singleRolesData = useSelector((state: TRootState) => state.rolesAndPermission?.singleRolesData);
  ...
  useEffect(() => {
    ...
    if (singleRolesData?.permissions && id !== "new") {
      ...
      console.log("modifiedData propane ");
  ...
  useEffect(() => {
    if (modifiedData?.length <= 1) {
      ...
      const cloneModuleData = _.cloneDeep(moduleData ? moduleData : []);
